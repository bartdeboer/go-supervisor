...
const commandName = "supervisorctl"
...
func main() {
	if code := runMain(os.Args[1:], os.Stdout, os.Stderr, os.Getenv); code != 0 {
		os.Exit(code)
	}
}

func runMain(args []string, stdout, stderr io.Writer, getenv func(string) string) int {
	a := app{
		configPath: supervisor.ConfigPath("", getenv),
		out:        stdout,
		err:        stderr,
	}
	if err := run(context.Background(), a, args); err != nil {
		fmt.Fprintln(stderr, err)
		fmt.Fprintln(stderr)
		_ = newRouter(a).FPrintHelp(context.Background(), stderr, nil)
		return 2
	}
	return 0
}
...
func newRouter(a app) *clir.Router {
	...
	r.Routes(func(b *clir.Builder) {
		...
		b.Route("service", func(b *clir.Builder) {
			...
			b.Describe("enable", fmt.Sprintf("Enable or replace one service.\n\nUsage:\n  %s service enable [--name <name>] [--cwd <dir>] [--restart never|on-failure|always] [--stop-timeout-ms <ms>] [--env KEY=VALUE] -- <command> [args...]\n\nChanges are written to the durable config. Run `%s reload` to apply them to the running supervisord.", commandName, commandName))
			...
			b.Describe("remove <name>", fmt.Sprintf("Remove one configured service from the durable config.\n\nUsage:\n  %s service remove <name>\n\nRun `%s reload` to apply the removal to the running supervisord.", commandName, commandName))
		})
		...
		b.Describe("reload", fmt.Sprintf("Signal PID 1 supervisord to reload its durable config.\n\nUsage:\n  %s reload", commandName))
	})
	return r
}
...
func enableService(a app, args []string) error {
	fs := flag.NewFlagSet(commandName+" service enable", flag.ContinueOnError)
	fs.SetOutput(a.err)
	...
	if len(argv) == 0 {
		return fmt.Errorf("missing service argv; use: %s service enable [flags] -- <command> [args...]", commandName)
	}
	...
	fmt.Fprintf(a.out, "run `%s reload` to apply\n", commandName)
	return nil
}
