...
// Main runs the supervisord control CLI and returns its process exit code.
func Main(command string, args []string, stdout, stderr io.Writer, getenv func(string) string) int {
...
func enableService(a app, args []string) error {
	fs := flag.NewFlagSet(a.command+" service enable", flag.ContinueOnError)
	fs.SetOutput(a.err)
	...
	if len(argv) == 0 {
		return fmt.Errorf("missing service argv; use: %s service enable [flags] -- <command> [args...]", a.command)
	}
